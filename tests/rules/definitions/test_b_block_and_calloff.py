...
def test_bipr_negative_no_licence_and_boundary():
    spec = load_rule(
        "core/rules/uk/definitions/b_block/01_bipr_perimeter_and_license.yaml"
    )
...
def test_bipr_positive_clear_boundary_and_licence():
    ...
    t = (
        "Foreground excludes Background Intellectual Property. Company has a perpetual, worldwide, royalty-free licence "
...
def test_bribe_negative_missing_ukba7_poca_fcpa_flowdown():
    ...
    assert out is not None and any(
        "ukba coverage weak" in f.message.lower()
        or "poca" in f.message.lower()
        or "flow-down"
...
def test_bribe_positive_full_coverage():
    spec = load_rule("core/rules/uk/definitions/b_block/02_bribe_ukba_poca_fcpa.yaml")
    t = (
        "Anti-Bribery Laws include the UK Bribery Act 2010 (including section 7 failure to prevent and adequate procedures), "
...
def test_calloff_negative_no_exclusion_or_precedence():
    ...
    assert out is not None and any(
        "exclusion" in f.message.lower() or "precedence" in f.message.lower()
        for f in out.findings
    )
...
def test_calloff_negative_performance_no_controls():
    spec = load_rule(
        "core/rules/uk/calloff/02_calloff_formation_by_performance_controls.yaml"
    )
    t = "A Call-Off becomes effective upon commencement of Work."
    out = run_rule(spec, AI(t, clause="call-off"))
    assert out is not None and any(
        "acceptance by performance" in f.message.lower()
        or "precedence"
        in (f.suggestion.text.lower() if getattr(f, "suggestion", None) else "")
        for f in out.findings
    )
...
def test_calloff_negative_minimum_content():
    ...
    out = run_rule(spec, AI(t, clause="call-off"))
    assert out is not None and any(
        "minimum elements" in f.message.lower() for f in out.findings
    )
