...
def test_site_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/01_site_ownership_access_risk.yaml"
    )
...
def test_site_positive():
    ...
    t = (
        "Site means locations owned, leased, licensed or under the control of the Company or its Affiliates; "
...
def test_specs_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/02_specifications_versioning_variations.yaml"
    )
    ...
    assert out is not None and any(
        "dynamic updates" in f.message.lower() or "dcc" in f.message.lower()
...
def test_specs_positive():
    ...
    t = (
        "Specifications are DCC-controlled (revision/date). Any update requires a Variation Order under Clause 14; "
...
def test_subcontractor_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/03_subcontractor_flowdown_audit.yaml"
    )
...
def test_subcontractor_positive():
    ...
    t = (
        "Subcontractor means any party at all tiers (tier-n), including suppliers of equipment and materials; "
...
def test_supplied_software_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/04_supplied_software_scope_licence_escrow.yaml"
    )
...
def test_supplied_software_positive():
    ...
    t = (
        "Supplied Software includes executables, embedded components, utilities, keys/passwords/diagnostics, documentation and updates/patches; "
...
def test_tariff_code_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/05_tariff_code_classification.yaml"
    )
...
def test_taxes_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/06_taxes_matrix_withholding_grossup.yaml"
    )
    t = "Taxes means taxes."
...
def test_taxes_positive():
    ...
    t = (
        "A responsibility matrix allocates Contractor Group taxes (payroll/NI/import/export duties/fees); "
...
def test_third_party_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/07_third_party_crtpa_alignment.yaml"
    )
...
def test_trade_tariff_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/08_trade_tariff_source_recency.yaml"
    )
...
def test_tupe_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/09_tupe_eli_requirements.yaml"
    )
    t = "TUPE applies."
...
def test_vo_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/11_variation_order_contents.yaml"
    )
...
def test_vo_positive():
    ...
    t = (
        "A Variation Order includes scope delta, Time Impact Analysis, price build-up, interfaces and effects on warranties/LDs, "
...
def test_vor_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/12_variation_order_request_process.yaml"
    )
    t = "A VOR may be sent."
    out = run_rule(spec, AI(t, clause="variation"))
    assert out is not None and len(out.findings) >= 1
...
def test_vat_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/13_vat_registration_place_of_supply_zero_rating.yaml"
    )
...
def test_vat_positive():
    ...
    t = (
        "VAT refers to the Value Added Tax Act 1994; Contractor provides VAT registration/number and valid tax invoices; "
...
def test_work_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/14_work_scope_boundary_interfaces.yaml"
    )
...
def test_work_positive():
    ...
    t = (
        "Work includes explicit battery limits, interfaces and exclusions; QA/inspection regime applies and LDs may apply; "
...
def test_worksite_negative():
    spec = load_rule(
        "core/rules/uk/definitions/s_to_w_block/15_worksite_inclusion_exclusions.yaml"
    )
...
def test_worksite_positive():
    ...
    t = (
        "Worksite includes locations of execution and excludes manufacturing/pre-delivery plants unless expressly listed; "
