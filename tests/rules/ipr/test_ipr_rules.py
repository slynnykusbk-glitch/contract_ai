...
def test_bg_licence_conflict_fail():
    txt = "Contractor grants a perpetual, irrevocable licence to its Background IPR, provided that such licence is only for the duration of the Agreement."
...
def test_indemnity_remedies_pass():
    txt = "Contractor shall indemnify Company for any IPR claim and shall procure a licence or replace or modify the infringing item to provide equivalent performance at no additional cost and no adverse impact on other works or interfaces, subject to this Agreement."
    res = _run(txt, "ipr.indemnity.remedies")
    assert res is None
...
def test_supplied_software_definition_ok():
    txt = "Supplied Software includes software, embedded firmware, activation keys, test scripts, patches and updates, and manuals."
    res = _run(txt, "ipr.supplied_software.definition")
    assert res is None
