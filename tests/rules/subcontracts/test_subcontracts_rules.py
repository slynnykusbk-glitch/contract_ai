...
def test_subcontracts_prior_consent_negative():
    spec = load_rule(
        "core/rules/subcontracts/subcontracts_universal.yaml",
    ...
    assert out and any(
        "prior written consent" in f.message.lower() for f in out.findings
    )
...
def test_flowdown_negative_missing_all():
    ...
    assert (
        "confidentiality" in msgs.lower()
        or "ip" in msgs.lower()
        or "audit" in msgs.lower()
    )
...
def test_flowdown_positive_all_present():
    ...
    t = (
        "Subcontractors shall comply with anti-bribery (Bribery Act), sanctions and export control (incl. ownership and control test), "
...
def test_art28_positive_all_terms_present():
    ...
    t = (
        "A sub-processor may process personal data subject to Article 28 processor terms: controller instructions, security, audit/inspection, "
...
def test_step_in_missing_crtpa():
    ...
    out = run_rule(spec, AI(t, "subcontracts"))
    assert out and any(
        "CRTPA" in " ".join(f.legal_basis) or "collateral" in f.suggestion.text.lower()
