...
def test_t3_marking_register_warn_when_missing_register():
    text = (
        "Supplier shall mark materials as 'Property of Company' and keep them separate."
    )
...
def test_t4_delivery_up_access_pass():
    text = (
        "Upon termination or insolvency, Company shall have the right to enter Supplier's premises and any warehouse "
...
def test_t5_title_risk_warn_when_no_link():
    ...
    assert res and any(
        "without" in f.message.lower() and "risk" in f.message.lower()
...
def test_t6_embedded_software_licence_pass():
    text = (
        "Embedded software shall be licensed to Company on a perpetual, royalty-free basis, sublicensable within the "
    ...
    assert res and any(
        "perpetual" in f.message.lower() or "licence" in f.message.lower()
        for f in res.findings
    )
