...
- **Backend (FastAPI):**
  ...
  - CORS увімкнено для локальної панелі (localhost/127.0.0.1) + `Origin: null` (self-test).
...
> Використовуй `https://localhost:9000` (або додай 127.0.0.1 у SAN сертифіката).
...
```
Перевірка: https://localhost:9000/health → `{"status":"ok","rules":[...],"count":N}`.

### 2.3 Front (панель)
- HTTPS сервер на 3000 (наприклад, `http-server -S`).
- URL бекенда в панелі: **https://localhost:9000**.

### 2.4 Self-test
Відкрити `word_addin_dev/panel_selftest.html` → Backend URL `https://localhost:9000` → **Run**.
...
- Порожня сторінка /health → відкрито `https://127.0.0.1:9000`, а сертифікат на `localhost`. Відкрий `https://localhost:9000` або перевипусти серт із SAN:127.0.0.1.
...
## 8) Команди-шпаргалка
- Бекенд: див. 2.2
- Self-test: file:///.../word_addin_dev/panel_selftest.html → Backend=https://localhost:9000
